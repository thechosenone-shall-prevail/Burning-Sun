...
// CF API definitions (manual to avoid cfapi.h dependency)
...
void InitCldApi() {
    ...
    if (hNtdll) {
        _WakeByAddressAll = (VOID(WINAPI*)(PVOID))GetProcAddress(hNtdll, "WakeByAddressAll");
        _WaitOnAddress = (BOOL(WINAPI*)(PVOID, PVOID, SIZE_T, DWORD))GetProcAddress(hNtdll, "WaitOnAddress");
    }
}
...
retry:
    ...
    CloseHandle(hlk);
    if (_WakeByAddressAll) _WakeByAddressAll(&gevent);
    CloseHandle(gevent);
    gevent = NULL;
...
int main()
{
    ...
    if (!CreateDirectoryW(workdir, &sa))
    {
    ...
    if (_WaitOnAddress) _WaitOnAddress(&gevent, &gevent, sizeof(HANDLE), INFINITE);
